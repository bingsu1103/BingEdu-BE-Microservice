...
const createQuestion = async (question) => {
    try {
        ...
        if (id !== result.data._id || level !== result.data.level || (question.question_type === "multiple_choice" && result.data.type === "essay") || (question.question_type === "essay" && result.data.type === "multiple_choice")) {
            return {
                status: false,
                EC: 1,
...
const createMultipleQuestion = async (listQuestions) => {
    try {
        ...
        if (invalidQuestions.length === listQuestions.length) {
            console.log("✅ ALL INVALID — ready to return");
            return {
                status: false,
                EC: 1,
        ...
        return {
            status: true,
            EC: 0,
            message: invalidQuestions.length > 0 ? 'Some questions failed to create' : 'Questions created successfully',
            data: { validQuestions, invalidQuestions, questionIds }
        };
    } catch (error) {
...
module.exports = { createQuestion, createMultipleQuestion, updateQuestion, getAllQuestion, getQuestion, getQuestionByLessonId, deleteQuestion }