...
const register = async (userData) => {
    try {
        ...
        const refresh_token = generateRefreshToken(newUser.data._id);

        await Auth.findOneAndUpdate(
            { user_id: newUser.data._id },
            { $set: { refresh_token: refresh_token, expire: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000) } },
            { upsert: true, new: true }
        );

        const emailResult = await emailUntil.sendWelcomeEmail(newUser.data.email, newUser.data.name);
...
const refreshAccessToken = async (refreshToken) => {
    try {
        ...
        const tokenRecord = await Auth.findOne({ refresh_token: refreshToken });
        if (!tokenRecord) {
            return {
                status: false,
                EC: -1,
...
const forgotPassword = async (email) => {
    try {
        ...
        await Auth.findOneAndUpdate({ user_id: user._id }, { $set: { otp: otp, otp_expire: new Date(Date.now() + 10 * 60 * 1000) } });

        const emailResult = await emailUntil.sendResetPasswordEmail(user.email, user.name, otp);
...
const verifyOTP = async (email, otp) => {
    try {
        ...
        if (!authRes.otp || !authRes.otp_expire || authRes.otp !== otp || authRes.otp_expire < Date.now()) {
            return {
                status: false,
                EC: 1,
...
const resetPassword = async (email, newPassword) => {
    try {
        ...
        const authRes = await Auth.findOneAndUpdate({ user_id: dataRes.data.data }, { $set: { otp: undefined, otp_expire: undefined } });
        if (!authRes) {
            return {
                status: false,
                EC: 1,
...
module.exports = { register, login, refreshAccessToken, logout, forgotPassword, fetchAccount, verifyOTP, resetPassword }